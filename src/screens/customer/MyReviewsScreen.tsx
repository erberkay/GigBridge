...
export default function MyReviewsScreen({ navigation }: any) {
  const [reviews, setReviews] = useState(DEMO_MY_REVIEWS);
  const [editModal, setEditModal] = useState<typeof DEMO_MY_REVIEWS[0] | null>(null);
  const [editComment, setEditComment] = useState('');
  const [editRating, setEditRating] = useState(5);

  const openEdit = (review: typeof DEMO_MY_REVIEWS[0]) => {
    setEditModal(review);
  ...
  const saveEdit = () => {
    ...
    setReviews((prev) => prev.map((r) => r.id === editModal!.id ? { ...r, comment: editComment.trim(), rating: editRating } : r));
    setEditModal(null);
  };
  ...
  const deleteReview = (id: string) => {
    Alert.alert('Yorumu Sil', 'Bu yorumu silmek istediğinize emin misiniz?', [
      ...
      { text: 'Sil', style: 'destructive', onPress: () => setReviews((p) => p.filter((r) => r.id !== id)) },
    ]);
  };
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            <Ionicons name="star-outline" size={48} color={Colors.textMuted} style={{ marginBottom: 16 }} />
        ...
        renderItem={({ item }) => (
          <View style={styles.reviewCard}>
            <View style={styles.reviewTop}>
              <LinearGradient colors={getAvatarGrad(item.targetName)} style={[styles.targetAvatar, item.type === 'venue' && { borderRadius: 12 }]}>
            ...
            <View style={styles.starsRow}>
              ...
              <View style={[styles.typeBadge, { backgroundColor: item.type === 'venue' ? Colors.venueColor + '22' : Colors.artistColor + '22' }]}>
                ...
                <Text style={[styles.typeText, { color: item.type === 'venue' ? Colors.venueColor : Colors.artistColor }]}>
        ...
        <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.ratingRow}>
              {[1, 2, 3, 4, 5].map((s) => (
                <TouchableOpacity key={s} onPress={() => setEditRating(s)} style={{ padding: 4 }}>
